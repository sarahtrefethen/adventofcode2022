...
def part1():
    ...
    with open("D11input.txt") as input:
        ...
        [parse_monkey(index, monkey) for index, monkey in enumerate(raw_monkies)]
...
def part2():
    ...
    magic_monkey_multiplier = reduce(lambda a, b: a*b, [monkey["test_int"] for monkey in monkeys])

    for n in range(10000):
        for monkey in monkeys:
            for old in monkey['items']:
                monkey['counter'] += 1
                new_worry = eval(monkey['inspect']) % magic_monkey_multiplier
                new_monkey = monkey['test'](new_worry)
                monkeys[new_monkey]['items'].append(new_worry)
            monkey['items'] = []
